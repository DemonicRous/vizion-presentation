<template>
    <div class="slide-tasks">
      <!-- Анимация под-слайдов – как основные, но по вертикали -->
      ...
      <div class="sub-nav">
        <button
          class="sub-nav-btn up"
          :class="{ disabled: subIndex === 0 }"
          @click="changeSub(-1)"
          :disabled="subIndex === 0"
        >
          ▲
        ...
        <button
          class="sub-nav-btn down"
          :class="{ disabled: subIndex === subCount - 1 }"
          @click="changeSub(1)"
          :disabled="subIndex === subCount - 1"
        >
          ▼
        </button>
      </div>
  ...
  import Overview from './SlideTasks/Overview.vue'
  import Stats from './SlideTasks/Stats.vue'
  import Demo from './SlideTasks/Demo.vue'
  
  const route = useRoute()
  const router = useRouter()
  
  const subIndex = computed(() => parseInt(route.query.sub) || 0)
  const subComponents = [Overview, Stats, Demo]
  const subCount = subComponents.length
  const currentComponent = computed(() => subComponents[subIndex.value] || Overview)
  ...
  /* Родитель с минимальной высотой – предотвращает «схлопывание» */
  .content-wrapper-relative {
    position: relative;
    width: 100%;
    max-width: 1400px;
    margin: 0 auto;
    min-height: 65vh;           /* фиксируем минимальную высоту, чтобы не прыгал контент */
  }
  ...
  /* ===== ПЛАВНАЯ АНИМАЦИЯ (как у обычных слайдов, но по вертикали) ===== */
