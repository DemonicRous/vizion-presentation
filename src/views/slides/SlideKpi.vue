<template>
  ...
  import Overview from './SlideKpi/Overview.vue'
  import Example from './SlideKpi/Example.vue'
  import Stats from './SlideKpi/Stats.vue'
  
  const route = useRoute()
  const router = useRouter()
  
  const subIndex = computed(() => parseInt(route.query.sub) || 0)
  const subComponents = [Overview, Example, Stats]
  ...
  <style scoped>
  /* стили такие же, как в SlideTasks, SlideAudits – унифицированные */
  .slide-kpi {
    min-height: 100vh;
    width: 100%;
    display: flex;
