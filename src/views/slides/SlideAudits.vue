<template>
  ...
  import Overview from './SlideAudits/Overview.vue'
  import Stats from './SlideAudits/Stats.vue'
  import PdfReport from './SlideAudits/PdfReport.vue'
  
  const route = useRoute()
  const router = useRouter()
  
  const subIndex = computed(() => parseInt(route.query.sub) || 0)
  const subComponents = [Overview, Stats, PdfReport]   // ← порядок изменён
